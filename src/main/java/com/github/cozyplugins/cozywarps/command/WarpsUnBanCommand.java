    ...
    public @Nullable CommandStatus onPlayer(@NotNull PlayerUser user, @NotNull ConfigurationSection section, @NotNull CommandArguments arguments) {
        ...
        if (user.hasPermission("cozywarps.staff")) {
            ...
            if (arguments.getArguments().size() < 2) {
                user.sendMessage("&7Please select a warp owner and a player to unban.");
            ...
            user.sendMessage("&7The player &f" + playerName + " &7has been unbaned from &f" + ownerName + "'s &7warps.");
            return new CommandStatus();
        }

        // Check for the correct number of arguments.
        if (arguments.getArguments().isEmpty() || arguments.getArguments().get(0).isEmpty()) {
            user.sendMessage("&7Please provide a name to unban from your warps.");
            return new CommandStatus();
        }
        ...
        user.sendMessage("&7The player &f" + playerName + " &7has been unbanned from your warps.");
        return null;
    }
